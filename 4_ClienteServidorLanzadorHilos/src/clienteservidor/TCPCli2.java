 ...
 */
public class TCPCli2 {

	public static void main(String[] args) {
		int nCli;

		if (args.length == 3) { // 2 parametros: nombre de la maquina remota a la que se conecta y puerto por el
								// que escucha dicha máquina.

			nCli = Integer.parseInt(args[2]);

			try {
				Socket misocket = new Socket(args[0], Integer.parseInt(args[1]));
				// Socket enlazado al puerto fijado en la maquina args[0]
				String nombre = "salida" + nCli + ".txt";
				PrintWriter writer = new PrintWriter(nombre, "UTF-8");
				
				//System.out.println("Cliente conectado");
				writer.println("Cliente conectado");
				
				BufferedReader lectura;
				//BufferedReader lee_datos;
				...
				while (!msg.equals(".")) {
					...
					if (i < 10)
						msg = "Prueba con mayusculas " + nCli;
					else
						msg = ".";
					i++;
					...
					// Para simular varios clientes en paralelo, lo escribimos en un fichero
					try {
						writer.println("Mensaje del servidor: " + lectura.readLine());
					} catch (Exception e) {
						e.printStackTrace();
					}
				...
				writer.println("Mensaje de despedida del servidor: " + lectura.readLine());
